...
class Jwt extends JwtEnv
{
  ...
  public static function createBearer(string $token): string
  {
    return "Bearer " . $token;

  }

  public static function parseBearer(string $authorization): string|false
  {
  ...
  public static function isExpired(string $token) {
    ...
    if ($expire > 0) {
      $created = Jwt::getPayload($token)["_created"];
      $interval =
